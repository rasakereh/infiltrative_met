...
def extract_features(src, stains, labels_im, biggest_comps, feature_list, brain_boundary_pts, max_cmp_cnt=20):
    ...
    for comp in tqdm(biggest_comps[selected_indices]):
        ...
        for curr_rect in all_rects:
            ...
            seg[:, :, 2] += cv2.rectangle(empty_canvas, int_rect[0:2], int_rect[0:2] + int_rect[2:4], .6, 45)
            axs[0, 1].imshow(seg)
...
def tumor_components(obj_segments, h_pix_size, w_pix_size, brain_boundary_pts, cache):
    ...
    # axs[1].imshow(found_components)
    # axs[1].set_title('Found components')
    # plt.show()
...
def interface_ellipse(obj_segments, h_pix_size, w_pix_size, brain_boundary_pts, cache):
    ...
    for cnt in contours:
        ...
        detected_interface = cv2.drawContours(detected_interface, [cnt], -1, 255, 20)

    interfaced_seg = obj_segments.copy()
    interfaced_seg[detected_interface != 0] = .1
    ...
    axs[0].imshow(obj_segments)
    axs[0].set_title('Segments')
    axs[1].imshow(interfaced_seg)
    axs[1].set_title('Ellipse Approximation')
    plt.show()
...
for src in src_list:
    ...
    for i in range(1):
        ...
        axs[1, 0].imshow(seg)
        axs[1, 0].set_title('Tumor Interface')
