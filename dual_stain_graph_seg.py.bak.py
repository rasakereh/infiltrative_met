...
def read_slide(src, rect=None, size=None, thumbnail_size=None):
    ...
    cropped_w = (int(thumbnail.shape[1] * .01), int(thumbnail.shape[1] * .8))
    cropped_h = (int(thumbnail.shape[0] * .01), int(thumbnail.shape[0] * .8))
    ...
    return {'main': image, 'thumbnail': thumbnail}
...
def color_analysis(image):
    ...
    brain_mask2 = cv2.dilate(brain_mask2, kernel3x3, iterations = 1)
    brain_mask2 = cv2.erode(brain_mask2, kernel2x2, iterations = 1)
    brain_mask2 = cv2.dilate(brain_mask2, kernel4x4, iterations = 1)
    ...
    # fig.savefig('seg_res/%s.png'%pref) # plt.show()

    return tumor_mask, brain_mask

def find_tumors(tumor_mask, CMP_PROPORTION = .75):
...
def extract_features(labels_im, biggest_comps, feature_list, brain_boundary_pts, max_cmp_cnt=50):
    all_features = []
    ...
    sel_ubound = min(100, len(biggest_comps))
    ...
    for comp in tqdm(biggest_comps[selected_indices]):
        ...
        if mask_area(curr_im, None, None) < 10:
            break

        curr_features = calculate_features(curr_im, feature_list, brain_boundary_pts)
...
def calculate_features(object_mask, feature_list, brain_boundary_pts, return_hidden=False):
    res = {}
    for feature_func in feature_list:
        feature_val = feature_func(object_mask, brain_boundary_pts, res)
...
######## Features
def _brain_mask(obj_mask, brain_boundary_pts, cache):
    return brain_mask

def _mask_curve(obj_mask, brain_boundary_pts, cache):
    contours, _ = cv2.findContours(obj_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    return contours

def _mask_curvature(obj_mask, brain_boundary_pts, cache):
    if '_mask_curve' in cache:
        contours = cache['_mask_curve']
    else:
        contours = _mask_curve(obj_mask, brain_boundary_pts, cache)
...
def _mask_convex_hull(obj_mask, brain_boundary_pts, cache):
    if '_mask_curve' in cache:
        contours = cache['_mask_curve']
    else:
        contours = _mask_curve(obj_mask, brain_boundary_pts, cache)
...
def mask_perimeter(obj_mask, brain_boundary_pts, cache):
    if '_mask_curve' in cache:
        contours = cache['_mask_curve']
    else:
        contours = _mask_curve(obj_mask, brain_boundary_pts, cache)
...
def mask_area(obj_mask, brain_boundary_pts, cache):
    return np.where(obj_mask != 0)[0].shape[0]

def mask_compactness(obj_mask, brain_boundary_pts, cache):
    m_perim = cache['mask_perimeter'] if 'mask_perimeter' in cache else mask_perimeter(obj_mask, brain_boundary_pts, cache)
    m_area = cache['mask_area'] if 'mask_area' in cache else mask_area(obj_mask, brain_boundary_pts, cache)
    compactness = np.pi * 4 * m_area / m_perim / m_perim

    return compactness

def mask_convexity(obj_mask, brain_boundary_pts, cache):
    m_perim = cache['mask_perimeter'] if 'mask_perimeter' in cache else mask_perimeter(obj_mask, brain_boundary_pts, cache)
    m_cnx_hull = cache['_mask_convex_hull'] if '_mask_convex_hull' in cache else _mask_convex_hull(obj_mask, brain_boundary_pts, cache)
    m_cnx_perim = mask_perimeter(obj_mask, brain_boundary_pts, {'_mask_curve': [m_cnx_hull]})

    convexity = m_cnx_perim / m_perim

    return convexity

def mask_solidity(obj_mask, brain_boundary_pts, cache):
    m_area = cache['mask_area'] if 'mask_area' in cache else mask_area(obj_mask, brain_boundary_pts, cache)
    m_cnx_hull = cache['_mask_convex_hull'] if '_mask_convex_hull' in cache else _mask_convex_hull(obj_mask, brain_boundary_pts, cache)
...
def bending_energy(obj_mask, brain_boundary_pts, cache):
    if '_mask_curvature' in cache:
        curvature = cache['_mask_curvature']
    else:
        curvature = _mask_curvature(obj_mask, brain_boundary_pts, cache)
    
    return np.mean(np.power(curvature, 2))

def total_abs_curv(obj_mask, brain_boundary_pts, cache):
    if '_mask_curvature' in cache:
        curvature = cache['_mask_curvature']
    else:
        curvature = _mask_curvature(obj_mask, brain_boundary_pts, cache)
...
def nearest_boundary_score(obj_mask, brain_boundary_pts, cache):
    if '_mask_curve' in cache:
        curr_boundary = cache['_mask_curve']
    else:
        curr_boundary = _mask_curve(obj_mask, brain_boundary_pts, cache)
    curr_boundary = curr_boundary[0].reshape((-1, 2))
    
    all_dist_quans = []
    for boundary_pt in curr_boundary:
        distances = np.linalg.norm(brain_boundary_pts - boundary_pt.reshape((1, 2)), axis=1)
...
def convex_overlap(obj_mask, brain_boundary_pts, cache):
    if '_brain_mask' in cache:
        brain_mask = cache['_brain_mask']
    else:
        brain_mask = _brain_mask(obj_mask, brain_boundary_pts, cache)
    
    if '_mask_convex_hull' in cache:
        m_cnx_hull = cache['_mask_convex_hull']
    else:
        m_cnx_hull = _mask_convex_hull(obj_mask, brain_boundary_pts, cache)
    ...
    overlaping_area = mask_area(brain_overlap, None, None)
    tumorfree_hull_area = mask_area(cnx_filled, None, None) - mask_area(obj_mask, None, None)
...
def surroundedness_degree(obj_mask, brain_boundary_pts, cache):
    if '_brain_mask' in cache:
        brain_mask = cache['_brain_mask']
    else:
        brain_mask = _brain_mask(obj_mask, brain_boundary_pts, cache)
    ...
    if '_mask_curve' in cache:
        ...
        mask_curve = _mask_curve(obj_mask, brain_boundary_pts, cache)
    ...
    v_list = np.array([__v(tumor_point, selected_brain) for tumor_point in mask_curve[0].reshape(-1, 2)])
    ...
    return np.amin(v_list)
    

def enclosing_circle_overlap(obj_mask, brain_boundary_pts, cache):
    if '_brain_mask' in cache:
        brain_mask = cache['_brain_mask']
    else:
        brain_mask = _brain_mask(obj_mask, brain_boundary_pts, cache)
    ...
    overlaping_area = mask_area(brain_overlap, None, None)
    circle_mask_area = mask_area(circled_mask, None, None)
...
for src in src_list:
    ...
    for i in range(1):
        ...
        curr_img = read_slide(src, size=(8000, 0), thumbnail_size=(2000, 0))
        ...
        kernel2x2 = np.ones((2,2),np.uint8)
        kernel3x3 = np.ones((3,3),np.uint8)
        brain_mask_bound = cv2.erode(brain_mask, kernel3x3, iterations = 3)
        brain_mask_bound = cv2.dilate(brain_mask_bound, kernel3x3, iterations = 30)
        ...
        # labeled_img, labels_im, biggest_comps = find_tumors(tumor_mask)
        labeled_img, labels_im, biggest_comps = find_tumors(tumor_interface)
